...
class Swatches extends \Magento\Framework\View\Element\Template
{
    ...
    private function renderSwatch(Option\Value $value)
    {
        ...
        $attributes = [
            ...
            'data-id'      => $value->getId(),
            'data-option-type'  => $this->optionHelper->getSwatchFinalType($value),
            'data-option-label' => $this->optionHelper->getSwatchFinalTitle($value),
        ...
        switch ($this->optionHelper->getSwatchType($value)) {
            ...
            case Swatch::SWATCH_TYPE_VISUAL_COLOR:
                ...
                if ($this->optionHelper->hasTooltip($value->getOption())) {
                    $attributes['data-option-tooltip-value'] = $color;
            ...
            case Swatch::SWATCH_TYPE_VISUAL_IMAGE:
                ...
                if ($this->optionHelper->hasTooltip($value->getOption())) {
                    $thumb = $this->optionHelper->getSwatchTooltipImageUrl($value);
                    $attributes['data-option-tooltip-thumb'] = $thumb;
                    $attributes['option-tooltip-thumb'] = $thumb;
                }
                break;
        }
